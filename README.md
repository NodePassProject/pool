...
func main() {
    ...
    conn, err := clientPool.OutgoingGet("connection-id", 10*time.Second)
    if err != nil {
        // Handle error...
        return
    }
...
func main() {
    ...
    // Get a connection by ID with timeout (usually received from the server)
    timeout := 10 * time.Second
    conn, err := clientPool.OutgoingGet("connection-id", timeout)
    if err != nil {
        // Handle error...
        return
    }
...
- `string`: The connection ID generated by the server
- `net.Conn`: The connection object
...
// Get current active connection count (connections available in the pool)
...
- **Connection ID Generation:**
  - The server generates a 4-byte ID and sends it to the client after TLS handshake.
...
- **OutgoingGet Method:**
  ...
  - Validates connection activity before returning.
    ...
    clientPool := pool.NewClientPool(
        ...
        func() (net.Conn, error) {
            ...
            if err != nil {
                ...
                // Note: cannot access clientPool here due to initialization order
                // Use external error tracking instead
            }
            return conn, err
        },
    )
...
func main() {
    ...
    // Usage
    id, conn, err := getNextPool().IncomingGet(30 * time.Second)
    if err != nil {
        // Handle error...
        return
    }
...
func (pm *PoolManager) getConnectionWithRetry(maxRetries int) (string, net.Conn, error) {
    for i := 0; i < maxRetries; i++ {
        id, conn, err := pm.pool.IncomingGet(5 * time.Second)
        if err == nil && conn != nil {
            return id, conn, nil
        }
...
func (s *Server) goodHandler(w http.ResponseWriter, r *http.Request) {
    ...
    id, conn, err := s.apiPool.IncomingGet(10 * time.Second)
    if err != nil {
        // Handle error
...
func TestPoolIntegration(t *testing.T) {
    ...
    clientConn, err := clientPool.OutgoingGet(id, 5 * time.Second)
    require.NoError(t, err)
